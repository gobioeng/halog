...
def plot_trend(widget, df: pd.DataFrame, title_suffix: str = ""):
    """Enhanced trend plotting with FIXED label overlapping"""
    # Clear existing plot
    ...
    fig.suptitle(f"LINAC Water System Trends{title_suffix}", fontsize=12, fontweight='bold')
    ...
    try:
        ...
        if len(avg_df) > 1:
            ...
            if len(clusters) > 1:
                ...
                ax.set_xlabel("Time (days with compressed gaps)", fontsize=9, labelpad=10)
